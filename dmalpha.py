		...
		runtodo = dirneed[ncount]
                ...
                if highres==1:
                        for icount in range(len(tlist)):
                                time=tlist[icount]
                                print 'time', time
                                Rvir37alpha, xdata, ydata, mvir, ms=dmalpha(runtodo,time,Rrange=[0.3,0.7],timesRv=0)
...
if todo=='Malpha1251':
	alpha51list=[]
		...
		if highres==1:
                        for icount in range(len(tlist)):
                                time=tlist[icount]
                                Rvir12alpha, xdata, ydata, mvir, ms=dmalpha(runtodo,time,Rrange=[0.01,0.02],timesRv=1)
                                Rvir51alpha, xdata, ydata, mvir, ms=dmalpha(runtodo,time,Rrange=[0.005,0.01],timesRv=1)
