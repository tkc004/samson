...
#        reff = np.array((xhalf+yhalf+zhalf)/3.0)
	...
	if haloname[0]=='f':
		plt.plot(vmagl, effsur, label=labelname,marker=ms,ls='none', mfc='none',markeredgecolor=hc,ms=12)
	else:
		plt.plot(vmagl, effsur, label=labelname,marker=ms,ls='none',color=hc,ms=12)
...
plt.savefig('figures/vmag_effsur_x.pdf')
plt.clf()
