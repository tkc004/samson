...
def readsnapcr(sdir,snum,ptype,
    snapshot_name='snapshot',
    extension='.hdf5',
    h0=0,cosmological=0,skip_bh=0,four_char=0,
    header_only=0,loud=0,havecr=0, ic=0):
    loud=1 
    ...
    if (ptype==0):
        if flag_cooling > 0:
                return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml,'ne':nume,'nh':numh,'sfr':sfr,'z':metal,'cregy':cregy,'cregyl':cregyl, 'cregyg':cregyg, 'cregyd':cregyd, 'cregyp':cregyp, 'cregya':cregya, 'header':newheader};
	elif havecr==1:
		return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml,'cregy':cregy, 'header':newheader};
        elif havecr==5:
		return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml,'cregy':cregy,'cregyl':cregyl, 'cregyg':cregyg, 'cregyd':cregyd, 'cregyp':cregyp, 'cregya':cregya, 'header':newheader};
	else:
                return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml, 'header':newheader};
    if (ptype==4):
