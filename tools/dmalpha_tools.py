...
def dmalpha(runtodo,time,Rrange=[0.3,0.7],timesRv=0):
	...
	mlist=[]
        snaplist, timelist = readtime(firever=firever)
	...
	if usepep==0:
		...
		ms = np.interp(np.log(ascale),np.log(alist),mslist)
                ...
                halosA = read_halo_history_pep(rundir, Nsnap, halonostr=halostr,hubble=hubble,comoving=1,maindir=maindir,firever=firever)
